<template>
  <div>
    <div style="margin-bottom: 5px">
      <el-input v-model="name" placeholder="输入姓名" suffix-icon="el-icon-search" style = "width:200px"
    ...
    >
      ...
      <el-table-column prop="no" label="账号" width="250" align="center">
      </el-table-column>
      ...
      <el-table-column prop="roleId" label="职位" width="150" align="center">
        <template slot-scope="scope">
          <el-tag
              :type="scope.row.roleId === 0 ? 'info' :(scope.row.sex === 1 ? 'warning' : 'danger')"
              disable-transitions>{{scope.row.roleId === 0 ? '系统管理员' :(scope.row.roleId === 1 ? '管理员' : '员工')}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="phone" label="电话" width="250" align="center">
      </el-table-column>
      <el-table-column prop="opetare" label="操作" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog
        title="编辑管理员信息"
        :visible.sync="centerAnotherDialogVisible"
        width="28%"
        center>
      ...
      <span slot="footer" class="dialog-footer">
    <el-button @click="centerAnotherDialogVisible = false">取 消</el-button>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      centerAnotherDialogVisible:false,
      form:{
        id:'',
        name:'',
        age:'',
      ...
      rules:{
        ...
        password: [
          ...
          { pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]{8,16}$/,message: "8-16个字符，至少包含1个大写字母，1个小写字母和1个数字",trigger: 'blur'}
        ],
        age: [
          { required: true, message: '请输入年龄', trigger: 'blur' },
          {pattern: /^([1-9][0-9]*){1,3}$/,message: '年龄必须为正数字',trigger: 'blur'},
  ...
  methods:{
        ...
        if (valid) {
          this.$axios.post( this.$httpUrl + '/user/update',this.form1).then(res=>res.data).then(res=>{
            ...
            if (res.code == 200) {
              ...
              this.centerAnotherDialogVisible = false
              this.loadPost()
            }else{
              this.$message({
                message: '操作失败!',
    ...
    mod(row){
      this.centerAnotherDialogVisible = true
      this.$nextTick(()=>{
        //赋值到表单
